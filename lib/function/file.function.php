 ...
 * 传入需要读取的文件夹路径,为程序编码
 */
function path_list($dir,$list_file=true,$check_children=false,$param=array()){
	$fileModel = Ebh::app()->model('file');
	if ($param['path'] == '/') {
	    unset($param['fileid']);
		$param['upid'] = 0;
	} else if(!empty($param['fileid'])) {
	    $param['upid'] = intval($param['fileid']);
	    unset($param['fileid']);
    } else {
		$path = substr($param['path'], 0,-1);
		if (is_numeric($path)) {
			$param['upid'] = $path;
		} else {
			$fileInfo = $fileModel->getOneFile(array('path'=>$param['path'],'uid'=>$param['uid'],'crid'=>$param['crid']));
			if (!empty($fileInfo)){
				$param['upid'] = $fileInfo['fileid'];
			} else {
	...
	$files = $fileModel->getFileList($param);
	...
	if (!empty($param['isdir'])) {
		if (!empty($files)) {
			...
			$parentList = $fileModel->getFilePidByPids(substr($pidStr, 0, -1));//查看是否有子目录
			$isParentArr = array();
			foreach ($parentList as $pid) {
				$isParentArr[] = $pid['upid'];
			}
		}
	}
	//获取所有文件
	if (!empty($files)) {
	    $shareList = Ebh::app()->model('Shareing')->getSimpleShareingList($param['uid'], $param['crid'], true);
        $uids = array_column($files, 'uid');
        $uids = array_unique($uids);
        $userinfos = Ebh::app()->model('User')->getUserInfoByUid($uids, true);
        unset($uids);
		foreach ($files as &$file) {
            $username = '';
            if (isset($userinfos[$file['uid']])) {
                $username = isset($userinfos[$file['uid']]['realname']) ? $userinfos[$file['uid']]['realname'] : $userinfos[$file['uid']]['username'];
            }
		    if (isset($shareList[$file['fileid']])) {
                $file['meta_info'] = 'path_self_share';
            }
			$file['atime'] = $file['dateline'];
			$file['ctime'] = $file['dateline'];
			$file['mtime'] = $file['dateline'];
			$file['mode'] = 'drwx rwx rwx(0777)';
			$file['is_readable'] = 1;
            $file['is_writeable'] = 1;
            $file['sourceid'] = $file['sid'];
            $file['sid'] = $file['fileid'];
            $file['this_uid'] = 0;
            $file['username'] = $username;
            if (!$list_file) {
            	if ($file['isdir'] && $file['upid'] == $param['upid']) {
					$file['isParent'] = in_array($file['fileid'],$isParentArr) ? true : false;
					$file['type'] = 'folder';
					if (empty($param['isFromSearch']))
						$file['path'] = '/'.$file['name'];
					$folderlist[] = $file;
				}
            } else {
            	if ($file['isdir']) {
					...
					if (empty($param['isFromSearch'])) {
						...
						if ($file['path'] != '/')
							$file['path'] = '/'.ltrim($file['path'],$param['path']);
					}
					$folderlist[] = $file;
				} else {
					//$file_link = APPHOST.'index.php?explorer/fileProxyA&path='.rawurlencode($this->in['path']);
					$file['isParent'] = false;
					$file['type'] = 'file';
					if (!empty($param['isFromSearch']) ) {//搜索的用代理输出url
						//$file['path'] = '/'.ltrim(rtrim($file['path'],'/').'/'.$file['name'],$param['path']);
						$file['path'] = '/'.ltrim(rtrim($file['path'],$file['name']).'/'.$file['name'],$param['path']);
					} else {
						$file['path'] = '/'.$file['name'];
					}
					$filelist[] = $file;
