...
class user extends EbhController{
	public $user=NULL;  //用户相关信息
	...
	function __construct(){
		...
		$this->user['role'] = 1;
		$this->user['config'] = array('size_max'=>1,'size_use'=>1);
		$this->user['group_info'] = array(1=>'write');
		$this->user['status'] = 1;*/
		/*$this->user['user_id'] = 1;
		$this->user['user_id'] = 1;
		$this->user['user_id'] = 1;*/
		//print_r($this->user);exit;
		//$_SESSION['kod_user']['name'] = $this->user['username'];
		//print_r($_SESSION['kod_user']);exit;
	...
	public function loginCheck(){
		...
		/*if(isset($_SESSION['kod_login']) && $_SESSION['kod_login']===true){*/
			$user = system_member::get_info(1);//默认先都是admin
	...
	private function login_success($user){
		...
		//$this->config['user']  = fileCache::load(USER.'data/config.php');这个之后可以从redis读取
	...
	public function common_js(){
		...
		//注入用户信息
		$the_config['crid'] = $this->crid;
		$the_config['uid'] = $this->user['uid'];
		$the_config['auth'] = urlencode(EBH::app()->getInput()->cookie('auth'));
		
		//print_r($_COOKIE);
		//print_r($GLOBALS['L']);exit;
